...
import br.wagnermorais.gestao__vagas.modules.candidate.useCases.ApplyJobCandidateUseCase;
import br.wagnermorais.gestao__vagas.modules.candidate.useCases.CreateCandidateUseCase;
import br.wagnermorais.gestao__vagas.modules.candidate.useCases.ListAllJobsByFilterUseCase;
import br.wagnermorais.gestao__vagas.modules.candidate.useCases.ProfileCandidateUseCase;
...
public class CandidateControler {
    ...
    @GetMapping("/")
    @PreAuthorize("hasRole('CANDIDATE')") // somente candida que possuem essarole como autorização 
    ...
    @GetMapping("/job")
    @PreAuthorize("hasRole('CANDIDATE')") // somente candida que possuem essarole como autorização 
    @Operation(summary = "Listagem de vagas disponíveis para o candidato" , description = "Essa função é responsável  por listar todas vagas disponíveis baseada no filtro")
    @ApiResponses({
        @ApiResponse(responseCode = "200" , content =  {
            @Content(
                array = @ArraySchema(
    ...
    @SecurityRequirement(name = "jwt_auth")
    @Operation(summary = "Inscrição do candidato para uma vagas" , description = "Essa função é responsável  fazer a inscrição do candidato a uma vaga")
